...
def process_machine_unit(machine_id, row_idx):
    try:
        ...
        important_features = {
            'id': str(machine_id),
            'timestamp': str(test_df['timestamp']),  # Ensure timestamp is string
...
def process_all_machines():
    ...
    processed_count = 0
    for pointer in pointers:
        # Change these lines to use index access
        machine_id = pointer[0]  # First column (id)
        row_idx = pointer[1]     # Second column (row_idx)
